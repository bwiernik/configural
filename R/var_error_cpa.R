...
#' @param Rsq_adj The adjusted R-squared value used to estimate sampling covariances for parameters adjusted for overfitting.
#'
...
#'           n = 17060)
#' }
var_error_cpa <- function(Rxx, rxy, n, se_var_mat = "normal", Rsq_adj = NULL) {
  Rxx <- as.matrix(Rxx)
  p <- ncol(Rxx)
  Rinv <- solve(Rxx)
  Id <- diag(p)
  ...
  if (p == 1) {
    ...
    dlevpat2.drxy <-
      ((t(one) %*% Rxx %*% Q %*% Rinv) / (t(bstar) %*% Rxx %*% bstar %*% t(one) %*% Rxx %*% one)) -
      2 * ((t(one) %*% Rxx %*% one %*% t(bstar) %*% Rxx %*% one) /
             (t(bstar) %*% Rxx %*% bstar %*% t(one) %*% Rxx %*% one)^2) *
      (t(bstar) %*% Rxx %*% Q %*% Rinv)
    2 * ((t(rxy) %*% bstar) / (t(bstar) %*% Rxx %*% bstar)) %*%
      (t(rxy) %*% Q %*% Rinv + t(bstar) -
         (solve((t(bstar) %*% Rxx %*% bstar)) %*% (t(rxy) %*% bstar %*% t(bstar) %*% Rxx %*% Q %*% Rinv) ))
    ...
    v.levpat2 <- j.levpat2 %*% Sigma %*% t(j.levpat2)
